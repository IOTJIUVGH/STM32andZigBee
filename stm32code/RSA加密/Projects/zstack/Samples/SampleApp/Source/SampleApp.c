   ...
   // unsigned long int e=0,n=0;
     case SAMPLEAPP_COM_CLUSTERID:     //如果是串口透传的信息
      ...
      if(r==0)
      {
        ...
        unsigned long int e=0,n=0;
        unsigned long int *e1,*n1;
        e1=(unsigned long int *)&pkt->cmd.Data[5];
        e=*(e1);
        n1=(unsigned long int *)&pkt->cmd.Data[9];
        n=*(n1);
       /* for(i=8;i>4;i--)
        {
          e=e<<8;
          e=((unsigned long int)pkt->cmd.Data[i])|e;
        }
        for(i=12;i>8;i--)
        {
          n=n<<8;
          n=((unsigned long int)pkt->cmd.Data[i])|n;
        }
        */
        
        RSA_e=e;
        RSA_n=n;
        HalUARTWrite(0,"收到密钥",8);
      // for(i=0;i<4;i++)
      // HalUARTWrite(0,(unsigned char *)&RSA_e,4);
      // for(i=0;i<4;i++)
       //HalUARTWrite(0,(unsigned char *)&RSA_n,4);
      ...
      else
      {
        ...
        for(i=0;i<(len/4);i++)
        {
          p=(unsigned long int *)&pkt->cmd.Data[i*4+1];
           textRSA[i]=*(p);
        }
        for(i=0;i<(len/4);i++)
          text[i]=(uint8)RSA(textRSA[i],RSA_d,RSA_n);
        for(i=0;i<(len/4);i++)
        HalUARTWrite(0,&text[i],1);
      }

...
float temp[20],val,max,min,byte0,byte1;
u16 adcx;
...
u16 Get_Adc_Average(u8 ch,u8 times)//¼ÆËãÆ½¾ùÖµ
 {
     u32 temp_val=0;
     u8 t;
     for(t=0;t<times;t++)
...
float Analyze_Adc_U_Data()//¶ÁÈ¡µçÑ¹£¬ÏµÊýÊÇ·Å´óÒ»°Ù±¶µÄ
{
	u8 i;
//	float sub=0;
	u16 *flash_u;
	flash_u=(u16*)0X0800FC02;
	byte1=((float)(*flash_u))/100;
...
float Analyze_Adc_I_Data()//¶ÁÈ¡µçÁ÷£¬ÏµÊýÊÇ·Å´ó100±¶µÄ
{
	u8 i;
	//float sub=0;
	u16 *flash_i;
